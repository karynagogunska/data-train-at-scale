...
def preprocess_features(X: pd.DataFrame) -> np.ndarray:
    ...
    def create_sklearn_preprocessor() -> ColumnTransformer:
        ...
        distance_pipe = make_pipeline(
            ...
            FunctionTransformer(lambda dist: (dist - dist_min) /
                                (dist_max - dist_min)))
        ...
        time_pipe = make_pipeline(
            ...
            make_column_transformer(
                (
                    OneHotEncoder(categories=time_categories,
                                  sparse=False,
                                  handle_unknown="ignore"), [2, 3]
                ),  # correspond to columns ["day of week", "month"], not the others columns
                (FunctionTransformer(lambda year: (year - year_min) /
                                     (year_max - year_min)),
                 [4]),  # min-max scale the columns 4 ["year"]
