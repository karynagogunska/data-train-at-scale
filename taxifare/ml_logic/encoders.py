...
def transform_lonlat_features(X: pd.DataFrame) -> pd.DataFrame:
    ...
    def distances_vectorized(df: pd.DataFrame, start_lat: str, start_lon: str,
                             end_lat: str, end_lon: str) -> dict:
        """
        ...
        lat_1_rad, lon_1_rad = np.radians(df[start_lat]), np.radians(
            df[start_lon])
